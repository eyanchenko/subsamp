# subsamp

This repository includes the code for "Graph sub-sampling for divide-and-conquer algorithms in large networks" by Eric Yanchenko.  

samplers - code to implement the various sub-sampling algorithms

PACE - code to implement the PACE commmunity detection divide-and-conquer algorithm from Mukherjee et al. (2021)

CPDAC - code to implement the core-periphery divide-and-conquer algorithm from Yanchenko (2022)

oneshot_com.R, oneshot_CP.R - code to carry out the simulation study from the paper for community and core-periphery detection, respectively

packages.txt - required R version and packages to run the code

CPresults.zip, Comresults.zip - Results from paper used to generate plots


